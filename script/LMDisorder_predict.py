...
def save_text(seq, pred, name):
    with open('../example/result/{}.txt'.format(name),'w') as w1:
        w1.writelines("# Output of LMDisorder" + '\n')
        w1.writelines("Num" + '\t' + "AA" + '\t' +'Score' + '\n')
        for i in range(len(seq)):
            aa = seq[i]
            pred_aa = pred[i]
            w1.writelines("{}".format(i) + '\t' + "{}".format(aa) + '\t' + '{}'.format(round(pred_aa,3)) + '\n')
    return 0
...
def evaluate(model, data_loader,device):
    ...
    for data in data_loader:
        with torch.no_grad():
            ...
            seq_list = sequence_eva[0].replace('\n','')
            pred_list = y_pred[0].numpy().tolist()
            save_text(seq_list, pred_list, sequence_names[0])
